...
class CommandProcessor(object):
    ...
    def run(self):
        ...
        if cmd["type"] == "attack":
            ...
            usage = cmd['usage']
            usage = usage.replace("<FILE>", self.path)
            usage = usage.replace("<IP>", localhost)
            usage = usage.replace("<PORT>", str(target_port))
            r = requests.get(link)
            ...
            # 공격코드가 10초 이상 걸리거나, 패킷 사이의 간격이 2초 이상이라면 코드수정이 필요하다. 
            queue = Queue()
            lo_proxy = Process(target = utility.proxy, args=(target_port, ))            # 1
            lo_sniffer = Process(target = packet.local_sniffer, args=(target_port, queue))    # 2
